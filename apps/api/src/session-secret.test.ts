...
describe("resolveSessionSecret (audit C2)", () => {
  ...
  it("falls back (warned) when too short outside production, never silently truncating/using it", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const secret = resolveSessionSecret({ FOREMAN_SESSION_SECRET: "short" });
