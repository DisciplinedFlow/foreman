...
// audit C2: mirrors apps/api/src/session-secret.ts — this secret signs the
// manifest-flow `state` param (setup.ts), so the same fail-fast applies here.
// A silent `?? "dev-only-secret"` fallback meant a misconfigured production
// deploy would boot with a short, guessable, well-known secret and never
// notice. Now it's a loud choice: throw before accepting traffic when
